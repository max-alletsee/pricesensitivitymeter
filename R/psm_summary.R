...
summary.psm <- function(object, ...) {
  ...
  cat("---\n")
  cat(ifelse(object$validated == TRUE, object$total_sample - object$invalid_cases, object$total_sample), " cases with individual price preferences were analyzed (", ifelse(object$weighted == TRUE, "weighted", "unweighted"), " data)", sep = "")
  ...
  if(object$invalid_cases > 0) {
    ...
    if(object$validated == TRUE) {
      cat("(Removed: n = ", object$invalid_cases, " / ", round(object$invalid_cases / object$total_sample, digits = 0) * 100, "% of data)", sep = "")
    } else {
